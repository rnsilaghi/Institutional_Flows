...
def write_exposure_summary_txt(df: pd.DataFrame, path="exposure_vs_next_q_return.txt"):
...
def write_stats_txt(stats: dict, path="stats_summary.txt"):
    with open(path, "w", encoding="utf-8") as f:
...
def run_pipeline():
    ...
    # 2) holdings quarter dates -> prices_eod (84 calls max)
    run_update_prices(tickers=TICKERS, max_quarters=PRICE_QUARTERS_PER_TICKER)

    # 3) per-manager txt summaries
